...
/// <summary>
/// Quản lý giao diện thanh máu (Health Bar) cho Enemy (Orge, Frog, v.v.) hiển thị trong World Space Canvas.
/// Tự động cập nhật lượng máu, giữ thanh máu thẳng đứng không bị lật ngược (flip) khi quái quay đầu.
/// </summary>
public class EnemyHealthBar : MonoBehaviour
{
    ...
    [Header("=== THAM CHIẾU QUÁI ===")]
    [SerializeField] private OrgeController ogreController;
    ...
    private void Awake()
    {
        ...
        if (ogreController == null)
        {
            ogreController = GetComponentInParent<OrgeController>();
        }
        if (frogController == null)
        {
            frogController = GetComponentInParent<FrogController>();
        }
    }
    ...
    private void Start()
    {
        ...
        if (ogreController != null)
        {
            ogreController.OnHPChanged += HandleHPChanged;
            UpdateHealthImmediate(ogreController.MaxHP, ogreController.MaxHP); // Khởi tạo đầy máu
        }
        else if (frogController != null)
        {
            frogController.OnHPChanged += HandleHPChanged;
            UpdateHealthImmediate(frogController.MaxHP, frogController.MaxHP); // Khởi tạo đầy máu
        }
    }
    ...
    private void LateUpdate()
    {
        ...
        // Đặt vị trí tuyệt đối trong World Space dựa trên vị trí của quái cha + offset cố định.
        // Điều này giúp thanh máu đứng im một chỗ cố định so với quái vật, hoàn toàn không bị lệch
        // sang hai bên khi quái vật quay đầu (Flip làm đổi dấu localScale.x).
